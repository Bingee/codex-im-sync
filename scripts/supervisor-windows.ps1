<#
.SYNOPSIS
  Windows daemon manager for codex-im-sync bridge.
...
# ── Paths ──
$CtiHome    = if ($env:CTI_HOME) { $env:CTI_HOME } else { Join-Path $env:USERPROFILE '.codex-im-sync' }
...
function Install-WinSWService {
    ...
    # Run as current user so the service can access ~/.codex-im-sync and Codex login state
...
<service>
  <id>$ServiceName</id>
  <name>Codex IM Sync Bridge</name>
  <description>Codex IM Sync bridge daemon</description>
...
function Install-NSSMService {
    ...
    & $NSSMPath set $ServiceName Description "Codex IM Sync bridge daemon"
