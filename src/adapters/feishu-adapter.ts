...
type FeishuP2PEnteredEventData = {
  chat_id?: string;
  operator_id?: {
    open_id?: string;
    union_id?: string;
...
type FeishuCardActionEventData = lark.InteractiveCardActionEvent & {
  context?: {
    open_chat_id?: string;
  };
};
...
export class FeishuAdapter extends BaseChannelAdapter {
  ...
  /** Map operator open_id/user_id to the p2p chat_id for menu events. */
  ...
  async start(): Promise<void> {
    ...
    const dispatcher = new lark.EventDispatcher({}).register({
      ...
      'im.chat.access_event.bot_p2p_chat_entered_v1': async (data) => {
        this.handleP2PEnteredEvent(data as FeishuP2PEnteredEventData);
      },
      'application.bot.menu_v6': async (data) => {
        await this.handleMenuEvent(data as FeishuMenuEventData);
      },
  ...
  async send(message: OutboundMessage): Promise<SendResult> {
    ...
    const rawCard = (message as OutboundMessage & { feishuCard?: Record<string, unknown> }).feishuCard;
    if (rawCard) {
      const cardResult = await this.sendRawCard(message.address.chatId, rawCard);
  ...
  private async sendRawCard(chatId: string, card: Record<string, unknown>): Promise<SendResult> {
    try {
  ...
  private rememberUserChat(userId: string | undefined, chatId: string | undefined): void {
    if (!userId || !chatId) return;
    this.userChatIds.set(userId, chatId);
  }

  private resolveChatIdForUser(userId: string): string | null {
    return this.userChatIds.get(userId) || null;
  }

  private commandFromMenuEventKey(eventKey: string): string | null {
    ...
    const mapping: Record<string, string> = {
      ...
      cti_status: '/status',
      threads: '/threads',
      refresh_threads: '/refreshthreads',
      new_session: '/new',
      status: '/status',
    };
  ...
  private commandFromCardAction(data: FeishuCardActionEventData): string | null {
    const value = data.action?.value;
    if (!value || typeof value !== 'object') return null;

    const action = typeof value.cti_action === 'string' ? value.cti_action : '';
    if (action === 'refresh_threads') return '/refreshthreads';
    if (action === 'new_session') return '/new';
    if (action === 'show_status') return '/status';
    if (action === 'use_thread' && typeof value.sdk_session_id === 'string') {
      return `/use ${value.sdk_session_id}`;
    }
    return null;
  }

  private handleP2PEnteredEvent(data: FeishuP2PEnteredEventData): void {
    const userId = data.operator_id?.open_id || data.operator_id?.user_id || data.operator_id?.union_id;
    this.rememberUserChat(userId, data.chat_id);
  }

  private async handleMenuEvent(data: FeishuMenuEventData): Promise<void> {
    const userId = data.operator?.operator_id?.open_id
      || data.operator?.operator_id?.user_id
      || data.operator?.operator_id?.union_id
      || '';
    const eventKey = data.event_key || '';
    const command = this.commandFromMenuEventKey(eventKey);
    if (!userId || !command) return;

    const chatId = this.resolveChatIdForUser(userId);
    if (!chatId) {
      console.warn('[feishu-adapter] Menu event ignored: no chat mapping for user', userId);
      return;
    }

    this.enqueue({
      messageId: `menu:${eventKey}:${data.timestamp || Date.now()}`,
      address: {
        channelType: 'feishu',
        chatId,
        userId,
  ...
  private async handleCardActionEvent(data: FeishuCardActionEventData): Promise<Record<string, unknown> | void> {
    const userId = data.open_id || data.user_id || '';
    const chatId = data.context?.open_chat_id || this.resolveChatIdForUser(userId);
    const command = this.commandFromCardAction(data);
    try {
      getBridgeContext().store.insertAuditLog({
        channelType: 'feishu',
        chatId: chatId || 'unknown',
        direction: 'inbound',
        messageId: `card:${data.open_message_id}:${Date.now()}`,
        summary: `[CARD_ACTION] ${JSON.stringify({
          openMessageId: data.open_message_id,
          userId,
          actionTag: data.action?.tag,
          actionValue: data.action?.value,
          hasContextChatId: Boolean(data.context?.open_chat_id),
        })}`,
      });
    } catch { /* best effort */ }
    console.log('[feishu-adapter] Card action received:', JSON.stringify({
      openMessageId: data.open_message_id,
      userId,
      chatId,
      actionTag: data.action?.tag,
    ...
    if (!chatId || !command) {
      return {
        toast: {
          type: 'warning',
          content: 'Action not available. Check card callback settings.',
        },
      };
    }

    this.rememberUserChat(userId, chatId);
    this.enqueue({
      messageId: `card:${data.open_message_id}:${Date.now()}`,
      address: {
        channelType: 'feishu',
