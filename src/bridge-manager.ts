...
import type { BridgeStatus, ChannelAddress, InboundMessage, OutboundMessage, StreamingPreviewState } from 'claude-to-im/src/lib/bridge/types.js';
import { createAdapter, getRegisteredTypes } from 'claude-to-im/src/lib/bridge/channel-adapter.js';
...
import * as engine from 'claude-to-im/src/lib/bridge/conversation-engine.js';
import * as broker from 'claude-to-im/src/lib/bridge/permission-broker.js';
import { deliver, deliverRendered } from 'claude-to-im/src/lib/bridge/delivery-layer.js';
import { markdownToTelegramChunks } from 'claude-to-im/src/lib/bridge/markdown/telegram.js';
import { markdownToDiscordChunks } from 'claude-to-im/src/lib/bridge/markdown/discord.js';
...
import {
  ...
  sanitizeInput,
  validateMode,
} from 'claude-to-im/src/lib/bridge/security/validators.js';
import { findCodexThread, listCodexThreads, type CodexThreadSummary } from './codex-thread-index.js';
...
function buildCompactThreadLabel(thread: CodexThreadSummary, current: boolean, index: number): string {
  return `${index}. ${escapeHtml(thread.projectName)}${current ? ' [current]' : ''}`;
}

function buildCompactThreadHint(thread: CodexThreadSummary): string {
  return escapeHtml(thread.title);
}

function buildThreadsText(threads: CodexThreadSummary[], currentSdkSessionId: string): string {
  if (threads.length === 0) {
    return 'No local Codex threads found in <code>~/.codex/sessions</code>.';
  }

  const lines = [
    '<b>Recent Codex Threads</b>',
    'Use <code>/use 1</code>, <code>/use 2</code> ... to switch.',
    '',
  ];
  for (const [index, thread] of threads.entries()) {
    const current = currentSdkSessionId && currentSdkSessionId === thread.sdkSessionId ? ' [current]' : '';
    lines.push(`<b>${buildCompactThreadLabel(thread, Boolean(current), index + 1)}</b>`);
    lines.push(buildCompactThreadHint(thread));
  }

  lines.push('');
  lines.push('Use <code>/refreshthreads</code> to rescan local threads.');
  return lines.join('\n');
}

function buildFeishuThreadsCard(threads: CodexThreadSummary[], currentSdkSessionId: string, refreshed: boolean): Record<string, unknown> {
  const elements: Array<Record<string, unknown>> = [
    {
      tag: 'markdown',
      content: refreshed
        ? '**Recent Codex Threads**\nUse `/use 1`, `/use 2` ... to switch.'
        : '**Recent Codex Threads**\nUse `/use 1`, `/use 2` ... to switch.',
    },
  ];
  ...
  if (threads.length === 0) {
    elements.push({
      tag: 'markdown',
      content: 'No local Codex threads found in `~/.codex/sessions`.',
    });
  } else {
    for (const [index, thread] of threads.entries()) {
      const current = currentSdkSessionId && currentSdkSessionId === thread.sdkSessionId;
      elements.push({
        tag: 'div',
        text: {
          tag: 'lark_md',
          content: [
            `**${index + 1}. ${thread.projectName}${current ? ' [current]' : ''}**`,
            thread.title,
          ].join('\n'),
        },
      });
  ...
  elements.push({
    tag: 'markdown',
    content: 'Commands: `/refreshthreads`  `/status`  `/use <index>`',
  });
...
async function sendThreadListResponse(
  adapter: BaseChannelAdapter,
  address: ChannelAddress,
  currentSdkSessionId: string,
  forceRefresh = false,
): Promise<void> {
  const threads = listCodexThreads({ forceRefresh, limit: 8 });
  const message = {
    address,
    text: buildThreadsText(threads, currentSdkSessionId),
    parseMode: 'HTML',
  } as OutboundMessage & { feishuCard?: Record<string, unknown> };

  if (adapter.channelType === 'feishu') {
    message.feishuCard = buildFeishuThreadsCard(threads, currentSdkSessionId, forceRefresh);
  }

  await deliver(adapter, message);
}

function switchBindingToThread(address: ChannelAddress, sdkSessionId: string): CodexThreadSummary | null {
  const thread = findCodexThread(sdkSessionId);
  if (!thread) return null;

  const { store } = getBridgeContext();
  const existingBinding = router.resolve(address);
  const newSession = store.createSession(
    `Bridge: ${thread.projectName}`,
    existingBinding.model || '',
    undefined,
    thread.cwd,
    existingBinding.mode,
  );
  ...
  store.updateSdkSessionId(newSession.id, thread.sdkSessionId);
...
async function handleMessage(
  ...
  const rawText = normalizeFeishuMenuText(msg.text.trim());
  ...
  switch (command) {
    case '/start':
      response = [
        ...
        '/use &lt;thread_id&gt; - Switch to a local Codex thread',
        '/cwd /path - Change working directory',
        '/mode plan|code|ask - Change mode',
        '/status - Show current status',
        '/sessions - List recent sessions',
        '/threads - List local Codex threads',
        '/refreshthreads - Refresh local Codex threads',
    ...
    case '/use': {
      if (!args) {
        response = 'Usage: /use &lt;index|thread_id|project_name&gt;';
        break;
      }
      const thread = switchBindingToThread(msg.address, args);
      if (!thread) {
        response = 'Local Codex thread not found. Use /threads to see indexes, thread IDs, or project names.';
        break;
      }
      response = [
        '<b>Thread switched</b>',
        '',
        `Project: <b>${escapeHtml(thread.projectName)}</b>`,
        `Thread: <code>${thread.sdkSessionId}</code>`,
        `CWD: <code>${escapeHtml(thread.cwd)}</code>`,
      ].join('\n');
      break;
    }
    ...
    case '/threads': {
      const binding = router.resolve(msg.address);
      await sendThreadListResponse(adapter, msg.address, binding.sdkSessionId, false);
      return;
    }

    case '/refreshthreads': {
      const binding = router.resolve(msg.address);
      await sendThreadListResponse(adapter, msg.address, binding.sdkSessionId, true);
      return;
    }
...
function normalizeFeishuMenuText(rawText: string): string {
  const trimmed = rawText.trim();
  const normalized = trimmed.toLowerCase();
  const mapping: Record<string, string> = {
    cti_threads: '/threads',
    cti_threads_refresh: '/refreshthreads',
    cti_new_session: '/new',
    cti_status: '/status',
  };
  if (mapping[normalized]) return mapping[normalized];

  const slotMatch = normalized.match(/^(?:cti_)?use_(\d{1,2})$/);
  if (slotMatch) {
    return `/use ${slotMatch[1]}`;
  }
  return rawText;
}
